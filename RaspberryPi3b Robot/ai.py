...
#serArd = serial.Serial(port="/dev/ttys0", baudrate=9600)  # ttys0 GPIO14-TX, GPIO15-RX
#serArd.close()
#serArd.open()
...
model = Model(r'/home/dan/AI-linux/modules/EN')  # Module folder
...
cap = pyaudio.PyAudio()
stream = cap.open(format=pyaudio.paInt16, input_device_index=1, channels=1, rate=48000, input=True,
                  frames_per_buffer=8192) # # channels=2, rate=16000 for PSEye, channels=?, rate=44100 (or more) for other USB microphones
...
def reboot():
    ...
    r2.speak("ooo")
    os.system("python3 ~/AI-linux/roco-vosk.py")
    exit()
...
listening = False     # initial Awake state after power-on
sleeping = False
...
#    if sleeping == False & listening == False:
#
#        query = input(f"[Listening Off] {user}: ")
#        commands(query)
#
# ~~~~~~~~~~~~~~~~~~ Sleeping: ~~~~~~~~~~~~~~~~~~~~~~~~~~~~
#    if sleeping == True & listening == False:
#
